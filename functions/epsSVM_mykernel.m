function [model, out_param, kerneloption] = epsSVM_mykernel(instance_matrix,instance_matrixN, instance_matrixM,label_vector, in_param,cof1,cof2,cof3,flag)
...
kerneloption3 = max(max(max(instance_matrixN(:)), max(instance_matrix(:))),max(instance_matrixM(:)))-min(min(min(instance_matrix(:)),min(instance_matrixN(:))),min(instance_matrixM(:)));
...
K3 = makekernel(instance_matrixM,instance_matrixM,abs(max(instance_matrixM(:))-min(instance_matrixM(:))));

kerneloption = [abs(max(instance_matrix(:))-min(instance_matrix(:))) abs(max(instance_matrixN(:))-min(instance_matrixN(:))) abs(max(instance_matrixM(:))-min(instance_matrixM(:))) kerneloption3];
% switch flag
%     case 1
%        K=0.1*K1+0.2*K2+0.7*K3;   
%     case 2
